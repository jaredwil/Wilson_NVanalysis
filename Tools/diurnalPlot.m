...
%   Detailed explanation goes here
    ...
    [tmp1, numN1] = calcFeature_NV(session.data, ch ,'nlenergy', min, 1 ,label, Norm1 , hour,  1);
    [tmp2, numN2] = calcFeature_NV(session.data, ch ,'nlenergy', min, 1 ,label, Norm2 , hour,  1);
    
    
    [tmpA1, numA1] = calcFeature_NV(session.data, ch ,'nlenergy', minAust, 1 ,label2, Aust1 , hourAust,  1);
    tmpA1 = tmpA1(1:length(tmp1),:);
    numA1 = numA1(1:length(tmp1),:);
    
    [tmpA2, numA1] = calcFeature_NV(session.data, ch ,'nlenergy', minAust, 1 ,label2, Aust2 , hourAust,  1);
    tmpA2 = tmpA2(1:length(tmp1),:);
    numA2 = numA1(1:length(tmp1),:);
