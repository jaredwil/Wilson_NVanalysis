function [feat, numNan] = calcFeature_NV_Aust(datasets,channels,feature,winLen,dispPercent,outLabel,runIndex,blockLenSecs,parFlag,varargin)
...
if parFlag    %if flag is set do processing in parallel
    ...
    for i = 1:numel(datasets)
        ...
        parOffset = ceil(parBlocks*blockLenSecs*fs);
        ...
        parfor p = 1:numPar
            ...
            for j = 1:parBlocks
                %Get data
                startBlockPt = 1+(floor(blockLenSecs*(j-1)*fs))+ startSec*fs;
                startBlockPt = startBlockPt + parOffset*(p-1);
                endBlockPt = min(ceil(blockLenSecs*j*fs),ceil(numPoints))+ startSec*fs;
