function [feat, numNan] = calcFeature_NV_spdup(datasets,channels,feature,winLen,dispPercent,outLabel,runIndex,blockLenSecs,parFlag,varargin)
% This derivative of the calcFeature_NV function utilitzes the parfor loop
% to loop through the windows rather than the one hour chunks from the
% portal. This removes the need from multiple IEEG sessions and thus the
% bottle neck with the portal.
...
%         parNan = cell(numPar,1);
        ...
        %%%REC CHANGE
        featTot = cell(numBlocks,1);
        numNanTot = cell(numBlocks,1);
