...
numLam = 20;
lambda = linspace(0.1,100,numLam);
w         = zeros(size(trainFeats,2),numLam);
% wp      = zeros(numLam,1);
% numIter = zeros(numLam,1);
for lam = 1:length(lambda)
    
    disp(['Progress: '  num2str(lam) '/' num2str(length(lambda))]);
    [w(:,lam), ~, ~] = LassoBlockCoordinate(trainFeats,trainLabels,lambda(lam));
